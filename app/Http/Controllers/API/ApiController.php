...
class ApiController extends Controller
{
    public function get_city()
    {
        ...
        if (count($data) > 0) {
            ...
            $response['success'] = "1";
            $response['register'] = __("message.City_list");
            $response['data'] = $data;
        } else {
            $response['success'] = "0";
            $response['register'] = __("message.City_Not_Found");
        }
    ...
    public function showsearchdoctor(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data = Doctors::where('is_approve', '1')
                ->where('profile_type', '1') // ✅ Only doctors
                ->where('name', 'like', '%' . $request->get("term") . '%')
                ->select("id", "name", "address", "image", "department_id")
                ->paginate(100);
            if ($data) {
    ...
    //                     )
    //                     ->orderby('distance')->WhereNotNull("doctors.lat")->where('profile_type', 1)->where("is_approve", "1")->paginate(10);
    ...
    //                               * sin(radians(doctors.lat))) AS distance")
    //                     )
    //                     ->orderby('distance')->WhereNotNull("doctors.lat")->where('profile_type', 2)->where("is_approve", "1")->paginate(10);
    ...
    //                               * sin(radians(doctors.lat))) AS distance")
    //                     )
    //                     ->orderby('distance')->WhereNotNull("doctors.lat")->where('profile_type', 1)->where('city_id', $request->city_id)->where("is_approve", "1")->paginate(10);
    ...
    //                     "doctors.department_id",
    //                     "doctors.image",
    //                 )->where('profile_type', 2)->where('city_id', $request->city_id)->paginate(10);
    //                 if ($data) {
    //                     foreach ($data as $k) {
    //                         $k->image = asset("public/upload/doctors") . '/' . $k->image;
    //                         unset($k->department_id);
    ...
    public function nearbydoctor(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $query = DB::table("doctors")
                ...
                ->whereNotNull("doctors.lat")
                ->where("is_approve", "1");

            // Profile type handling
            if ($type == 1) {
                $query->where('profile_type', 1);
            } elseif ($type == 2) {
                $query->where('profile_type', 2);
            } elseif ($type == 3) {
                $query->where('profile_type', 3);
            }

            // Apply city filter if city_id is given and not 0
            if (!empty($city_id) && $city_id != 0) {
                $query->where("city_id", $city_id);
            }

            $data = $query->orderBy("distance")->paginate(10);
    ...
    public function getspeciality()
    {
        ...
        if (count($data) > 0) {
            foreach ($data as $d) {
                ...
                $d->total_doctors = Doctors::where("department_id", $d->id)
                    ->where("is_approve", '1')
                    ->count();
    ...
    public function viewdoctor(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->type == 1) {
                ...
                $getdetail = Doctors::where('id', $request->get("laboratory_id"))->where('profile_type', 3)->first();
                unset($getdetail->state);
                unset($getdetail->city);
                if (empty($getdetail)) {
                    $response['success'] = "0";
                    ...
                    $getdetail->avgratting = Review::where('doc_id', $request->get("laboratory_id"))->avg('rating');
                    $getdetail->total_review = count(Review::where('doc_id', $request->get("laboratory_id"))->get());
                    $getdetail->image = asset('public/upload/doctors') . '/' . $getdetail->image;
                    $response['success'] = "1";
                ...
                $getdetail = Doctors::where('id', $request->hospital_id)
                ...
                foreach ($hospitalDoctors as $doctor) {
                    $doctors[] = [
                        ...
                        'image' => asset('public/upload/doctors/' . $doctor->image),
                        'rating' => Review::where('doc_id', $doctor->id)->avg('rating'),
                        'total_review' => Review::where('doc_id', $doctor->id)->count(),
                    ];
                }

                unset($getdetail->state, $getdetail->city);

                $getdepartment = Services::find($getdetail->department_id);
                $getdetail->department_name = $getdepartment ? $getdepartment->name : "";

                $getdetail->avgratting = Review::where('doc_id', $getdetail->id)->avg('rating');
                $getdetail->total_review = Review::where('doc_id', $getdetail->id)->count();
                $getdetail->image = asset('public/upload/doctors/' . $getdetail->image);
    ...
    public function doctordetail(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->type == '1') {
                ...
                $data = Doctors::where('id', $request->get("laboratory_id"))->where('profile_type', '3')->orderBy('id', 'desc')->first();
                // echo "<pre>";
                // print_r($data);
                // die();
                if (empty($data)) {
    ...
    public function doctoreditprofile(Request $request)
    {
        ...
        $messages = array(
            'laboratory_id.required' => "laboratory_id.required",
            'pharmacy_id.required' => "pharmacy_id is required",
            'doctor_id.required' => "doctor_id is required",
            'name.required' => "name is required",
            'email.required' => "email is required",
            ...
            "home_visit.required" => "home visit is required",
            "lab_visit.required" => "lab visit is required",
        ...
        if ($validator->fails()) {
            ...
            if ($request->type == '1') {
                ...
                $store = Doctors::where('profile_type', '3')->where('id', $request->get("laboratory_id"))->first();
                if ($store) {
                    DB::beginTransaction();
                    try {
                        $img_url = $store->image;
    ...
    // start laboratory report api
    public function add_laboratory_report(Request $request)
    {
        $response = array("status" => 1, "msg" => "Validation error");
        ...
        $messages = array(
            'report_id.required' => "report_id is required",
            'laboratory_id.required' => "laboratory_id is required",
            'name.required' => "name is required",
            'description.required' => "description is required",
            'price.required' => "price is required",
        ...
        if ($validator->fails()) {
            ...
            $response['msg'] = rtrim($message, ', ');
        } else {
            if ($request->medicine_id == 0) {
                $data = new LaboratoryProduct();
                $msg = __("message.Successfull Report Added");
            } else {
                $data = LaboratoryProduct::find($request->report_id);
    ...
    public function laboratory_report(Request $request)
    {
        $response = array("status" => 0, "msg" => "Validation error");
        $rules = [
            "laboratory_id" => "required",
        ...
        if ($validator->fails()) {
            ...
            $setting = Setting::find(1);
            if ($request->keyword) {
                $data = LaboratoryProduct::where('laboratory_id', $request->laboratory_id)->where('name', 'LIKE', '%' . $request->keyword . '%')->get();
                if (count($data) != 0) {
                    $response['success'] = "1";
                    $response['msg'] = __("message.Report_Get_Success");
                    $response['data'] = $data;
                    $response['pharmacy_tax'] = $setting->pharmacy_tax;
                    $response['laboratory_delivery_charge'] = $setting->laboratory_delivery_charge;
                } else {
                    $response['success'] = "0";
                    $response['msg'] = __("message.Report_data_Not_Found");
                    $response['data'] = $data;
                    $response['laboratory_tax'] = $setting->laboratory_tax;
                    $response['laboratory_delivery_charge'] = $setting->labortory_delivery_charge;
                    // $response = array("status" => 0, "msg" => "Medicines data Not Found");
                }
            } else {
                $data = LaboratoryProduct::where('laboratory_id', $request->laboratory_id)->get();

                if (count($data) != 0) {
                    $laboratory = Doctors::find($request->laboratory_id); // add this line for home and lab visit display
                    ...
                    $response['pharmacy_tax'] = $setting->pharmacy_tax;
                    $response['pharmacy_delivery_charge'] = $setting->pharmacy_delivery_charge;
                    $response['home_visit'] = $laboratory->home_visit;
                    $response['lab_visit'] = $laboratory->lab_visit;
                } else {

                    $laboratory = Doctors::find($request->laboratory_id);
    ...
    public function delete_laboratory_report(Request $request)
    {
        $response = array("status" => 1, "msg" => "Validation error");
        $rules = [
            "report_id" => "required",
        ...
        if ($validator->fails()) {
            ...
            if ($data) {
                $data->delete();
                $response['status'] = "1";
                $response['msg'] = __("message.Successfull Report Delete");
            } else {
                $response['status'] = "1";
                $response['msg'] = __("message.Result Not Found");
            }
    ...
    public function report_order(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $response['status'] = "1";
            $response['msg'] = __("message.Report_Order_Success");
            $response['data'] = $data;
            // $android = $this->sendNotifications($msg, $request->get("doctor_id"), "doctor_id", $data->id);
    ...
    public function get_user_report_list(Request $request)
    {
        $response = array("status" => 1, "msg" => "Validation error");
        $rules = [
            "user_id" => "required",
        ...
        if ($validator->fails()) {
            ...
            $data = LaboratoryOrder::where('user_id', $request->user_id)
                ->orderBy('id', 'desc')
                ->get();

            if (!$data->isEmpty()) {
                foreach ($data as $d) {
                    $d->prescription = asset("public/upload/reportlist") . '/' . $d->prescription;
                    if ($d->order_type == 2) {

                        $reportDetails = LaboratoryOrderData::where('order_id', $d->id)->get();
                        $sub = 0;
                        if (!$reportDetails->isEmpty()) {
                            foreach ($reportDetails as $reportDetail) {
                                $report = LaboratoryProduct::find($reportDetail->service_id);

                                if ($report && $report->image) {
                                    $reportDetail->report_img = asset("public/upload/laboratoryreport") . '/' . $report->image;
                                } else {
                                    $reportDetail->report_img = "";
                                }
                                $sub += $reportDetail->price * $reportDetail->qty;
                            }
                        }
                        $d->report = $reportDetails; // Assigning the medicines to the order
                        $d->subtotal = $sub;
                    }



                    $laboratory = Doctors::find($d->laboratory_id);

                    if ($laboratory) {
                        $d->laboratory_name = $laboratory->name;
                        $d->laboratory_address = $laboratory->address;
                        $d->laboratory_email = $laboratory->email;
                        $d->laboratory_phoneno = $laboratory->phoneno;
                        $d->laboratory_image = $laboratory->image
                            ? asset("public/upload/doctors") . '/' . $laboratory->image
                            : "";
                    } else {
                        $d->laboratory_name = "Laboratory not found";
                        $d->laboratory_address = "";
                        $d->laboratory_email = "";
                        $d->laboratory_phoneno = "";
                        $d->laboratory_image = "";
                    }
                }
                ...
                $response['status'] = "1";
                $response['msg'] = __("message.Reoprt_List_Get_Success");
                $response['data'] = $data;
            } else {
                $response['status'] = "0";
                $response['msg'] = __("message.Report_not_found");
            }

            $response['status'] = "1";
            $response['msg'] = __("message.Prescription_Report_Success");
            $response['data'] = $data;
        }
        return json_encode($response, JSON_NUMERIC_CHECK);
    }
    ...
    public function view_report(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data = LaboratoryOrder::find($request->order_id);
            if ($data) {
                $data1 = LaboratoryOrderData::where('order_id', $data->id)->get();

                if (!$data1->isEmpty()) {
                    $sub = 0;
                    foreach ($data1 as $reportDetail) {
                        $report = LaboratoryProduct::find($reportDetail->service_id);

                        if ($report && $report->image) {
                            $reportDetail->laboratory_img = asset("public/upload/laboratoryreport") . '/' . $report->image;
                        } else {
                            $reportDetail->laboratory_img = "";
                        }
                        $sub += $reportDetail->price * $reportDetail->qty;
                    }
                    $data->subtotal = $sub;
                ...
                $laboratory = Doctors::find($data->laboratory_id);

                if ($laboratory) { // Check if the laboratory exists
                    $data->Laboratory_name = $laboratory->name;
                    $data->Laboratory_address = $laboratory->address;
                    $data->Laboratory_email = $laboratory->email;
                    $data->Laboratory_phoneno = $laboratory->phoneno;

                    $data->Laboratory_image = $laboratory->image
                        ? asset("public/upload/doctors") . '/' . $laboratory->image
                        : "";
                } else {
                    // Handle case if laboratory not found
                    $data->Laboratory_name = "Laboratory not found";
                    $data->Laboratory_address = "";
                    $data->Laboratory_email = "";
                    $data->Laboratory_phoneno = "";
                ...
                $response['status'] = "1";
                $response['msg'] = __("message.Report Details");
                $response['data'] = $data;
            } else {
                $response['status'] = "0";
                $response['msg'] = __("message.Report_not_found");
    ...
    public function get_laboratory_report_list(Request $request)
    {
        $response = array("status" => 1, "msg" => "Validation error");
        $rules = [
            "laboratory_id" => "required",
        ...
        if ($validator->fails()) {
            ...
            if ($request->type) {
                $data = LaboratoryOrder::where('laboratory_id', $request->laboratory_id)->whereIn('status', ['0', '1', '4', '5', ' 7', ' 8'])->orderby('id', "DESC")->get();
                if (count($data) != 0) {
                    foreach ($data as $d) {

                        $data1 = LaboratoryOrderData::where('order_id', $d->id)->get();
                        if (count($data1) != 0) {
                            $d->report = $data1;
                        }
                        $user = Patient::find($d->user_id);
                    ...
                    $response['status'] = "1";
                    $response['msg'] = __("message.Report_List_Get_Success");
                    $response['data'] = $data;
                } else {
                    $response['status'] = "0";
                    $response['msg'] = __("message.Report_not_found");
                ...
                $data = LaboratoryOrder::where('laboratory_id', $request->laboratory_id)->orderby('id', "DESC")->get();
                if (count($data) != 0) {
                    foreach ($data as $d) {
    ...
    public function change_laboratoryreport_status(Request $request)
    {
        $response = array("status" => 1, "msg" => "Validation error");
        $rules = [
            "order_id" => "required",
        ...
        if ($validator->fails()) {
            ...
            if ($data) {
                ...
                if ($request->get("status") == '1') { // accept
                    $msg = __("message.laboratory_order_Accept_msg");
                    $android = $this->sendNotifications($msg, $data->user_id, "user_id", $data->id);
                } else if ($request->get("status") == '2') { // cancel
                    $msg = __("message.laboratory_order_cancel_msg");
                    $android = $this->sendNotifications($msg, $data->user_id, "user_id", $data->id);
                } else if ($request->get("status") == '3') { // complete
                    $msg = __("message.laboratory_order_complete_msg");
                    $android = $this->sendNotifications($msg, $data->user_id, "user_id", $data->id);
                } else if ($request->get("status") == '4') { // in process
                    $msg = __("message.laboratory_order_inprocess_msg");
                    $android = $this->sendNotifications($msg, $data->laboratory_id, "doctor_id", $data->id);
                } else if ($request->get("status") == '5') { // sample collected
                    $msg = __("message.laboratory_order_sample_collected_msg");
                    $android = $this->sendNotifications($msg, $data->user_id, "user_id", $data->id);
                } else if ($request->get("status") == '6') { // absent
                    $msg = __("message.laboratory_order_absent_msg");
                    $android = $this->sendNotifications($msg, $data->user_id, "user_id", $data->id);
                } else if ($request->get("status") == '7') { // reject
                    $msg = __("message.laboratory_order_reject_msg");
                    $android = $this->sendNotifications($msg, $data->user_id, "user_id", $data->id);
                }

                $response['status'] = "1";
                $response['msg'] = __("message.Report_Status_Change_Success");
                // $response['data']=$data;
            } else {
                $response['status'] = "0";
                $response['msg'] = __("message.Report_not_found");
