...
public function getsitecurrenttime(){
    ...
    public function create_session(){
        $application_id = 7881;
        $auth_key = "TeJJLNPEa5TusbJ";
        $authSecret = "qDET9Zz4NONNHKk";
        $nonce = rand();
        $timestamp = time();
    ...
    if (isset($response['session']) && isset($response['session']['token'])) {
        return $response['session']['token'];
    } else {
        Log::error('ConnectyCube session or token missing', ['response' => $response]);
