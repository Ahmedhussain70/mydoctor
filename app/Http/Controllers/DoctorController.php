...
class DoctorController extends Controller
{
        ...
        return DataTables::of($doctors)
            ...
            ->editColumn('image', function ($doctors) {
                if ($doctors->image != "") {
                    ...
                    return asset("public/upload/doctors/defaultdoctor.png");
                }
            })
            ...
            ->editColumn('action', function ($doctors) {
                $edit = url('backend/savedoctor', array('id' => $doctors->id));
                $timeing = url('backend/doctortiming', array('id' => $doctors->id));
                $delete = url('backend/deletedoctor', array('id' => $doctors->id));
                $doctorapprove = url('backend/approvedoctor', array('id' => $doctors->id, "approve" => '1'));
                $txt = "";
                $setting = Setting::find(1);
    ...
    public function savedoctor($id)
    {
        ...
        $city = City::all();
        return view("admin.doctor.savedoctor")->with("id", $id)->with("data", $data)->with("department", $department)->with("setting", $setting)->with("city", $city);
    }
    ...
    public function reviewtable()
    {
        ...
        return DataTables::of($review)
            ...
            ->editColumn('action', function ($review) {
                $delete = url('backend/deletereview', array('id' => $review->id));
    ...
    public function doctordashboard(Request $request)
    {
        if (Session::get("user_id") != "" && Session::get("role_id") == '2') {
            ...
            //i add this code for redirect page to doctordashbord
            if ($doctordata->is_subscription == "1" || !$mysubscriptionlist) {
    ...
    public function postlogindoctor(Request $request)
    {
        ...
        if ($getUser) {
            ...
            if ($setting->doctor_approved == '1' && $getUser->is_approve == '1') {
               ...
               // i add this code for redirect page to doctordashbord
                // if ($getUser->is_subscription == 1 || !$mysubscriptionlist) {
                //     return redirect("doctordashboard");
                // } else {
                //     return redirect("doctor_subscription_plan");
                // }
    ...
    public function doctoreditprofile()
    {
        if (Session::get("user_id") != "" && Session::get("role_id") == '2') {
            ...
            $city = City::all();
            $doctordata = Doctors::with('departmentls')->find(Session::get("user_id"));
            return view("user.doctor.editprofile")->with("setting", $setting)->with("doctordata", $doctordata)->with("department", $department)->with("city", $city);
        } else {
            return redirect("/");
        }
    }
    ...
    public function doctor_add_plan(Request $request)
    {
        ...
        if ($request->get("payment_type") == "stripe") {
        ...
        else if ((int)$request->payment_type === 8) {
            $request->validate([
                'receipt_file' => 'required|mimes:jpeg,jpg,png,pdf|max:2048',
            ...
            $uploadPath = public_path('upload/bank_receipt');
            ...
            $data->subscription_id = $request->plan_id;
            $data->payment_type = 2;
            $data->amount = $request->amount;
            $data->deposit_image = $filename; // Save only filename
            $data->description = $request->receipt_description;
            $data->date = now();
            $data->status = 2;
            $data->is_complet = 1;
            $data->save();
    ...
    public function earningreports(Request $request)
    {
        $setting = Setting::find(1);
        $doctordata = Doctors::with('departmentls')->find(Session::get("user_id"));
        // $earningdata = BookAppointment::orderby('id', "DESC")->where("doctor_id", Session::get("user_id"))->where('is_completed', '1')->select("date", "id", "consultation_fees", "created_at")->get();
        ...
        // Apply the appropriate filter based on $data
        switch ($data) {
            case '1': // Custom range
        ...
        $query->where("doctor_id", $doctorId)
            ...
            ->orderby('id', "DESC")
            ->select("date", "id", "consultation_fees", "created_at",'user_id','slot_name')
            ->with('patientls');

        // Execute the query
        $earningdata = $query->get();
        $total = $query->sum('consultation_fees');

        return view('user.doctor.earningreports', compact('setting', 'doctordata', 'earningdata','total'));
    }
