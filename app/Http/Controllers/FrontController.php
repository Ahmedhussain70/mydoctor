 ...
 public function postforgotpassword(Request $request){
    ...
    public function searchdoctor(Request $request){
        ...
        if (!empty($term) && !empty($type)) { // 11
            $doctorslist = Doctors::with('departmentls')
        ...
        // For the map data
        if (!empty($term) && !empty($type)) { // 11
            $doctorslistmap = Doctors::with('departmentls')
                ->where("is_approve", "1")
                ->where("department_id", $type)
                ->where('name', 'like', '%' . $term . '%')
                ->where('profile_type', 1)
        ...
        // Additional processing for the doctor's list
        ...
        ->with("city_id",$city_id)
            ...
            ->with("doctorlist", $doctorslist)
            ->with("term", $term)
            ->with("type", $type)
            ->with("doctorslistmap", $doctorslistmap);
    }
   ...
   public function edit_about(Request $request){
    ...
    public function edit_terms(Request $request){
      ...
      $data->trems = $request->get('trems');
       $data->save();
      return redirect('backend/Terms_condition');
    }
    ...
    public function edit_app_privacy(Request $request){
      ...
      $data->privacy = $request->get('privacy');
       $data->save();
      return redirect('backend/app_privacy');
    }
    ...
    public function edit_data_deletion(Request $request){
      ...
      $data->data_deletion = $request->get('data_deletion');
       $data->save();
      return redirect('backend/data_deletion');
    }

    public function show_rejectuserappointment($id)
    {
        ...
        // Update status to rejected (example: 5 or another value)
