...
class PaymentController extends Controller
{
   ...
   public function show_pendingpaymenttable(){
        $book =DB::table('settlement')->select( DB::raw('DISTINCT(doctor_id) as doctor_id '),DB::raw('sum(amount) as amount'),DB::raw('count(*) as total_booking') )->where("status",'0')->groupBy('doctor_id')->get();
               // Retrieve the commission setting
        $setting = Setting::find(1);
        $commissionPercentage = $setting->commission;
           return DataTables::of($book)

            ->editColumn('doctor_name', function ($book) {
                return Doctors::find($book->doctor_id)?Doctors::find($book->doctor_id)->name:'';
            })
            ->editColumn('amount', function ($book) use ($commissionPercentage) {
                return $book->amount - ($book->amount * $commissionPercentage / 100); // Display amount after commission
            })
            ->editColumn('total_booking', function ($book) {
                return $book->total_booking;
            })
            ->editColumn('action', function ($book) {
                  $payurl=url('backend/payamount',array('doctor_id'=>$book->doctor_id));
   ...
   public function show_payamount($doc_id){
        $book =DB::table('settlement')->select( DB::raw('DISTINCT(doctor_id) as doctor_id '),DB::raw('sum(amount) as amount'),DB::raw('count(*) as total_booking') )->where("status",'0')->where("doctor_id",$doc_id)->groupBy('doctor_id')->first();
   ...
   public function updatepayment(Request $request){
        ...
        $store->save();
        DB::table('settlement')->where("status",'0')->where("doctor_id",$request->get("doctor_id"))->update(["status"=>1,"settlement_id"=>$store->id]);

        Session::flash('message',__("message.Payment Store Successfully"));
        Session::flash('alert-class', 'alert-success');
        return redirect('backend/pending_payment');
   }
