...
class PharmacyController extends Controller
{
    ...
    public function addpharmacy($id)
    {
        ...
        $city = City::all();
        return view("admin.pharmacy.savepharmacy")->with("id", $id)->with("data", $data)->with("setting", $setting)->with("city", $city);
    ...
    public function updatepharmacy(Request $request)
    {
        ...
        return redirect("backend/pharmacy");
    }
    ...
    public function searchpharmacy(Request $request)
    {
        ...
        if (!empty($term) && !empty($type)) { //11
            $doctorslist = Doctors::with('departmentls')->where("department_id", $type)->Where('name', 'like', '%' . $term . '%')->where("is_approve", "1")->where('profile_type', 2)->when($city_id, function ($query, $city_id) {return $query->where('city_id', $city_id);})->paginate(10);
        } else if (!empty($term) && empty($type)) { //10
            $doctorslist = Doctors::with('departmentls')->where("is_approve", "1")->Where('name', 'like', '%' . $term . '%')->where('profile_type', 2)->when($city_id, function ($query, $city_id) {return $query->where('city_id', $city_id);})->paginate(10);
        } else if (empty($term) && !empty($type)) { //01
            $doctorslist = Doctors::with('departmentls')->where("is_approve", "1")->where("department_id", $type)->where('profile_type', 2)->when($city_id, function ($query, $city_id) {return $query->where('city_id', $city_id);})->paginate(10);
        } else { //00
            $doctorslist = Doctors::with('departmentls')->where("is_approve", "1")->where('profile_type', 2)->when($city_id, function ($query, $city_id) {return $query->where('city_id', $city_id);})->paginate(10);
        }
        ...
        if (!empty($term) && !empty($type)) { //11
            ...
            $doctorslistmap = Doctors::with('departmentls')->where("is_approve", "1")->Where('name', 'like', '%' . $term . '%')->where('profile_type', 2)->when($city_id, function ($query, $city_id) {return $query->where('city_id', $city_id);})->get();
        } else if (empty($term) && !empty($type)) { //01
            $doctorslistmap = Doctors::with('departmentls')->where("is_approve", "1")->where("department_id", $type)->where('profile_type', 2)->when($city_id, function ($query, $city_id) {return $query->where('city_id', $city_id);})->get();
        } else { //00
        ...
        $city = City::all();
        return view("user.allpharmacy")->with("city_id",$city_id)->with("city", $city)->with("services", $services)->with("setting", $setting)->with("doctorlist", $doctorslist)->with("term", $term)->with("type", $type)->with("doctorslistmap", $doctorslistmap);
    ...
    public function pharmacydashboard(Request $request)
    {
        if (Session::get("user_id") != "" && Session::get("role_id") == '3') {
            ...
            foreach ($orderdata as $order) {
    $patient = Patient::find($order->user_id);
    $order->user_name = $patient ? $patient->name : 'Unknown';
}
    ...
    public function pharmacyeditprofile()
    {
        if (Session::get("user_id") != "" && Session::get("role_id") == '3') {
            ...
            $doctordata = Doctors::with('departmentls')->find(Session::get("user_id"));
            $city = City::get();
            return view("user.pharmacy.editprofile")->with("setting", $setting)->with("doctordata", $doctordata)->with("city", $city);
        } else {
            return redirect("/");
        }
    }
