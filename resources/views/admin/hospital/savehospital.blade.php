...
@section('title')
    {{ __('message.save') }} {{ __('message.Hospital') }} | {{ __('message.Admin') }} {{ __('message.Hospital') }}
@stop
@section('meta-data')
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-flex align-items-center justify-content-between">
                    <h4 class="mb-0">{{ __('message.save') }} {{ __('message.Hospital') }}</h4>
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="{{ url('backend/hospital') }}">{{ __('message.Hospital') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ __('message.save') }} {{ __('message.Hospital') }}</li>
                        </ol>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-8 col-md-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ url('backend/updatehospital') }}" class="needs-validation" method="post"
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control"
                                            placeholder='{{ __('message.Enter Hospital Name') }}' id="name"
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <textarea id="aboutus" class="form-control" rows="5" name="aboutus"
                                            placeholder='{{ __('message.Enter About Hospital') }}' required="">{{ isset($data->aboutus) ? $data->aboutus : '' }}</textarea>
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <div class="form-group">
                                    ...
                                    <div class="">
                                        ...
                                        <select name="department_id[]" id="department_id" class="form-control" multiple>
                                            ...
                                            @foreach ($department as $d)
                                                <option value="{{ $d->id }}" {{ in_array($d->id, $selected_departments) ? 'selected' : '' }}>{{ $d->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-12 col-md-12 col-sm-12 form-group mb-2">
                                    <div class="">
                                        <label>{{ __('message.Doctors') }}</label>
                                        <select name="doctor_id[]" id="doctor_id" class="form-control" multiple>
                                            @foreach ($doctors as $doc)
                                                <option value="{{ $doc->id }}" {{ in_array($doc->id, $assigned_doctors) ? 'selected' : '' }}>{{ $doc->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        if (typeof $ !== 'undefined' && $.fn.select2) {
            ...
            $('#department_id').select2({
                theme: 'bootstrap-5',
                width: '100%',
                placeholder: "{{ __('message.select') }} {{ __('message.Department') }}"
            });
            $('#doctor_id').select2({
                theme: 'bootstrap-5',
                width: '100%',
                placeholder: "{{ __('message.select') }} {{ __('message.Doctors') }}"
            });
        } else {
